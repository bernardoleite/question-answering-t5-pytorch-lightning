...
class QADataModule(pl.LightningDataModule):
    ...
    def setup(self):
        self.train_dataset = QADataset(self.train_df, self.tokenizer, self.max_len_input, self.max_len_output)
        self.validation_dataset = QADataset(self.val_df, self.tokenizer, self.max_len_input, self.max_len_output)
        self.test_dataset = QADataset(self.test_df, self.tokenizer, self.max_len_input, self.max_len_output)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description = 'Fine tune Model for Question Answering.')

    # Add arguments
    parser.add_argument('-dmn', '--dir_model_name', type=str, metavar='', default="qa_en_t5_base_512_96_32_6_seed_42", required=False, help='Directory model name.')
    ...
    parser.add_argument('-cd', '--current_date', type=str, metavar='', default=currentdate(), required=False, help='Current date.') # it is not being used for now
