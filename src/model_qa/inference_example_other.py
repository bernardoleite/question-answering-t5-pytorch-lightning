"""This is a work in progress script. Please, use inference_example.py"""
...
def output_predictions(predictions):
    table_results = pd.DataFrame(columns=['model_answer_prediction', 'model_returned_sequence', 'f1_score', 'exact_match_score'])
    print("Text: ", CONTEXT, "\n")
    ...
    for index, pred in enumerate(predictions):
        if len(GROUND_TRUTH_ANSWERS) > 0:
            ...
            f1_score = -1
            exact_match_score = -1
        table_results = table_results.append({'model_answer_prediction': pred, 'model_returned_sequence': index+1, 'f1_score': round(f1_score, 3), 'exact_match_score': exact_match_score*1}, ignore_index=True)
